...
a = a %>% mutate(Age_estimate = dbh/0.00707)
units(a$Age_estimate) = as_units(NULL) #если это убрать, будет работать криво
units(a$Age_estimate) = as_units("years")

a$Age_index = as.numeric(a$Age_index) #почему-то значения здесь были строковыми. Исправим это

Model_common = lm(data = a, log(as.double(biomass), base = exp(1)) ~ (as.double(Age_estimate)))

Model_betula = lm(data = a %>% filter(Species == "Betula pendula"),
                   log(as.double(biomass), base = exp(1)) ~ (as.double(Age_estimate)))

Model_acer = lm(data = a %>% filter(Species == "Acer platanoides"),
                  log(as.double(biomass), base = exp(1)) ~ (as.double(Age_estimate)))

Model_platanus = lm(data = a %>% filter(Species == "Platanus occidentalis"),
                  log(as.double(biomass), base = exp(1)) ~ (as.double(Age_estimate)))

Model_quercus = lm(data = a %>% filter(Species == "Quercus robur"),
                  log(as.double(biomass), base = exp(1)) ~ (as.double(Age_estimate)))

ggplot(data = a, aes(x = as.double(Age_estimate)^0.25, y=log(as.double(biomass), base = exp(1))))+
...
anova(Model_betula)
anova(Model_acer)
anova(Model_platanus)
anova(Model_quercus)
# Итоговые коэффициенты
summary(Model_common)
summary(Model_betula)
summary(Model_acer)
summary(Model_platanus)
summary(Model_quercus)
# Итоговые коэффициенты в доступном табличном виде
broom::tidy((Model_common))
broom::tidy((Model_betula))
broom::tidy((Model_acer))
broom::tidy((Model_platanus))
broom::tidy((Model_quercus))

biom_mod = function(a){lm(as.double(biomass) ~ as.double(Age_estimate), data = a)}
...
results %>% filter(r.squared < 0.999) %>% arrange(desc(r.squared)) %>% select(Site, r.squared)